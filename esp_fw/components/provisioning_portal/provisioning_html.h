...
static const char *RK_PROVISIONING_HTML =
    ...
    "<script>"
    "let stickySelectedSsid='';"
    "function renderWifiOptions(data){"
    "const select=document.getElementById('wifi_ssid');"
    "const targetSsid=stickySelectedSsid||select.value;"
    "let matched=false;"
    ...
    "if(targetSsid&&item.ssid===targetSsid){option.selected=true;matched=true;}select.appendChild(option);});"
    "if(targetSsid&&!matched){const fallback=document.createElement('option');"
    "fallback.value=targetSsid;fallback.textContent=targetSsid+' (当前选择的 Wi-Fi 暂未出现在本轮扫描结果中)';"
    "fallback.selected=true;select.appendChild(fallback);}"
    "}"
    ...
    "renderWifiOptions(data);"
    "status.textContent=data.length?('已刷新 '+data.length+' 个 Wi-Fi'):(stickySelectedSsid?'未扫描到可用 Wi-Fi，已保留当前选择':'未扫描到可用 Wi-Fi');"
    "}catch(err){status.textContent='Wi-Fi 扫描失败，3 秒后重试';console.error(err);}"
    "}"
    "document.getElementById('wifi_ssid').addEventListener('change',event=>{stickySelectedSsid=event.target.value;});"
    "refreshWifiList();"
